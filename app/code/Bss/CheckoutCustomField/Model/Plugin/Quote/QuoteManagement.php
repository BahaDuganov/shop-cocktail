...
class QuoteManagement extends \Magento\Quote\Model\QuoteManagement
{
    ...
    {
        $dataCustomField = [];
        $data = $this->request->getParam('bss_custom_field');
        if($data)
        {
            ...
            if(isset($data['paymentMethod']['extension_attributes']['bss_customfield'])){
                $dataCustomField = $data['paymentMethod']['extension_attributes']['bss_customfield'];
            }
        }
        ...
        $this->addBssCustomField($dataCustomField, $order);
