...
define([
    ...
    $.widget('mage.SwatchRenderer', {
        ...
        _RenderControls: function () {
            ...
            $.each(this.options.jsonConfig.attributes, function () {
                ...
                container.append(
                    ...
                    label +
                    '</span><div class="' + classes.attributeOptionsWrapper + ' clearfix">' +
                    options + select +
                    '</div>' + input +
                    '</div>'
                );
        ...
        _AppendQtyHtml: function (qty) {
            var html = '';
            html += '<select class="product-qty" id="qty" name="qty">';
            if (qty > 10) {
                for (var i = 1; i < 11; i++) {
                    html += '<option value="' + i + '">' + i + '</option>'
                }
                html += '<option value="11" class="custom-option">More</option>';
            }
            ...
            if ($('.custom-button').hasClass('hidden')) {
                $('.product-qty').replaceWith(html);
        ...
        _UpdatePrice: function () {
            ...
            if (result && result.oldPrice.amount !== result.finalPrice.amount) {
                $('div[data-product-id=' + $widget.options.jsonConfig.productId + ']' + ' ' + this.options.slyOldPriceSelector).show();
                // $('.'+$widget.options.jsonConfig.productId + ' ' +this.options.slyOldPriceSelector).show();
            } else {
