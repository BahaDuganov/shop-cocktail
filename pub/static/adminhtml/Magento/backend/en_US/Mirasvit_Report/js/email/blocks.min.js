define(['jquery','underscore','ko','uiComponent'],function($,_,ko,Component){'use strict';var Block=function(data){function guid(){function s4(){return Math.floor((1+Math.random())*0x10000).toString(16).substring(1);}
...
this.identifier.subscribe(fnc);this.timeRange.subscribe(fnc);this.limit.subscribe(fnc);this.isSubscribed=true;}.bind(this);};return Component.extend({defaults:{template:'report/email/blocks',blocks:[],ranges:[],reports:[],links:{email:'${ $.provider }:${ $.dataScope }'}},initialize:function(){var i;this._super();_.bindAll(this,'handleAdd','handleDelete','synchronize');if(this.email()){_.each(this.email().blocks,function(data){this.handleAdd(data);}.bind(this));}
