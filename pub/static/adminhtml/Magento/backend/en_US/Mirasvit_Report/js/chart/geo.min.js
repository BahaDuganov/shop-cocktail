define(['./abstract','jquery','underscore'],function(Abstract,$,_){return Abstract.extend({isLoaded:false,chartType:'geo',map:null,markers:[],ensureChart:function(){if(this.isLoaded){this._ensureChart();}else{require(['//www.gstatic.com/charts/loader.js?nomin','//maps.googleapis.com/maps/api/js?key=AIzaSyDOcoIgRmy7_yv_30OuqvZkulTwF2KJMiI&v=3.exp&signed_in=true&key=AIzaSyDOcoIgRmy7_yv_30OuqvZkulTwF2KJMiI'],function(){google.charts.load('current',{'packages':['geochart'],'mapsApiKey':'AIzaSyBr3pLR_c6dttTc4X1zYhTdduVEiktpRHM'});google.charts.setOnLoadCallback(function(){this.isLoaded=true;}.bind(this));}.bind(this));}},_ensureChart:function(){if(!document.getElementById('map_div')){return false;}
...
if(this.chartType==='map'&&this.map){_.each(this.markers,function(marker){marker.setMap(null);});this.markers=[];var latLngList=[];_.each(this.rows,function(row){var pos={lat:parseFloat(row['mst_reports_postcode|lat_orig']),lng:parseFloat(row['mst_reports_postcode|lng_orig'])};if(pos.lat&&pos.lng){latLngList.push(pos);var marker=new google.maps.Marker({position:pos,map:this.map,hint:'***'});this.markers.push(marker);}},this);this.getChartConfig();var bounds=new google.maps.LatLngBounds();for(var i=0,LtLgLen=latLngList.length;i<LtLgLen;i++){bounds.extend(latLngList[i]);}
