...
var $ul=$('<ul class="box-cate-link hidden-xs abs-dropdown">');$linkPlaceholder.empty();$ul.appendTo($linkPlaceholder).unwrap();var $mbTitle=$('<a href="javascript:void(0)" class="mobile-toggle visible-xs">');$mbTitle.insertBefore($ul);$('.tab-item',$tabs).each(function(id,el){var $tabItem=$(this),external=false,href='javascript:void(0)';if($tabItem.find('.tab-title').data('externalurl')){href=$tabItem.find('.tab-title').data('externalurl');external=true;}
var $tabTitle=replaceTag($tabItem.find('.tab-title'),'a').attr('href',href).removeAttr('data-externalurl');var $li=$('<li class="item">').addClass($tabTitle.data('class')).append($tabTitle.removeAttr('data-class'));$li.appendTo($ul);if(id==0){$li.addClass('active');$tabItem.addClass('active');$mbTitle.text($tabTitle.text());}
