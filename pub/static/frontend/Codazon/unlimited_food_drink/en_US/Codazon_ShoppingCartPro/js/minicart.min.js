...
this.cart[key](value);},this);let sid;var interval=setInterval(()=>{$('.cart-quantity-summery,.cart-quantity,.incart_qty').addClass('hide');$('.cart-item-qty').each(function(){var $span=$(this);sid=$span.attr('data-cart-item-id');console.log('update cart');$('#update_qty-'+sid).removeClass('hide');$('#summery-'+sid).removeClass('hide');$('#'+sid).removeClass('hide').text($(this).val());});clearInterval(interval);},1000);},updateCrosssell:function(crosssell){_.each(crosssell,function(value,key){if(!this.crosssell.hasOwnProperty(key)){this.crosssell[key]=ko.observable();}
