...
this.$catSearch.appendTo(this.element.find('form'));var dataSearch=this.$catSearch.data('search');$.codazon.categorySearch(dataSearch,this.$catSearch);self.$catSearch.find('.dropdown-inner').addClass('nice-scroll');}else{this.element.addClass('no-cat-search');}},_bindEvents:function(){var self=this,conf=this.options;this.$fullTrigger.on('click',function(){$('body').addClass('search-box-opened');if(!conf.enable){self.element.addClass('full-search-box');self.element.removeClass('no-full-box');}
