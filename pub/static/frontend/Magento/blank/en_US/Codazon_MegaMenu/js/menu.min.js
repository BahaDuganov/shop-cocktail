...
self._menuTabs();self.element.removeClass('no-loaded');$('body').trigger('cdzmenu.initialized',[this.element]);},_isToggleMenu:function(){return this.element.hasClass(this.options.toggleMenuClass);},_hideOverflowItems:function(){var self=this;var $ul=self.element.children('.groupmenu:first');$originalLis=$ul.children();self.overFlowItems=[];var $staticWrap=$('<div class="static-wrap" style="position: static;" />');$staticWrap.insertBefore(self.element);self.element.appendTo($staticWrap);var $controlWrap=$('<div style="display:none" class="switchpage-control" style="display:none;" />');var $control=$('<a class="control" href="#"><span class="dot"></span><span class="dot"></span><span class="dot"></span><span class="times"></span></a>');var $overFlowUl=$('<ul style="display:none" class="groupmenu overflow-items-container">');$controlWrap.appendTo($staticWrap);$control.appendTo($controlWrap);$overFlowUl.insertAfter($ul);function getOverFlowItems(){var ulWidth=$ul.innerWidth();var lisWidth=0;var overFlowItems=[];var overflow=false;$originalLis.each(function(id,el){var $li=$(this);lisWidth=lisWidth+$li.outerWidth(true);if(overflow){overFlowItems.push($li.get(0));}else{if(lisWidth>ulWidth){overFlowItems.push($li.get(0));overflow=true;}else if(lisWidth+60>ulWidth){if(($li.next().length)&&($li.outerWidth(true)>60)){overFlowItems.push($li.get(0));overflow=true;}}}});return overFlowItems;}
